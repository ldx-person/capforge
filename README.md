...
| `capforge update [name]` | 检查远端更新并增量拉取（默认 ff-only）；`--all` 更新全部已导入项目 |
...
- 扫描输出会包含：`SPDX/来源/置信度` + `改造支持(是否允许)` + `原因` + `reminders`。
- **默认策略是保守的**：不做法律判断，只做“风险提示 + 默认门禁”。

### 分类与默认行为

- **未检测到明确许可证 / NOASSERTION / 未知许可证**：
  - 默认认为可能是 *All Rights Reserved* 或条款不明确
  - **默认禁止生成改造扫描/改造计划**
  - 建议先确认许可证文本或联系作者
- **强 copyleft（GPL-2.0/GPL-3.0/AGPL-3.0）**：
  - 为避免跨项目复用引入“传染性”要求
  - **默认禁止生成改造扫描/改造计划**（仍可扫描/生成能力描述）
- **弱 copyleft（LGPL/MPL）**：
  - 默认允许生成改造扫描，但会提示“链接/分发边界”风险
  - 建议在真正执行改造与分发前做合规审查
- **宽松许可证（MIT/Apache/BSD/CC0 等）**：
  - 默认允许改造
  - 仍会提示保留 LICENSE/NOTICE/版权声明

当你在新项目中引用该开源项目代码进行开发/分发时，也应在 `capability.md` 的“集成指南”中明确提醒许可证要求。

### 覆盖方式（不推荐）

如果你已经完成合规评审，但仍希望继续生成改造扫描，可使用：

```bash
npx capforge transform <project> --ignore-license
```
